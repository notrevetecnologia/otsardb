...
#if defined(__EMSCRIPTEN__)
    if (getentropy(bytes, sizeof(bytes)) != 0) return 0;
#else
 ...
 * - POSIX (Linux/macOS): the symbol is resolved at runtime via dlsym —
 * NULL when studio_window.c is absent and the runtime check picks the
 * fallback. (GCC's __attribute__((weak)) was used before, but macOS ld64
 * rejects a missing weak reference at static link time, so dlsym is the
 * portable POSIX mechanism.)
...
#else
typedef int (*otsardb_studio_window_open_fn)(int port, char *error, size_t error_cap);
#endif
...
#else
    void *sym = dlsym(RTLD_DEFAULT, "otsardb_studio_window_open");
    if (sym) {
        otsardb_studio_window_open_fn fn;
        memcpy(&fn, &sym, sizeof(fn)); /* avoid pedantic function-ptr cast */
        return fn(port, error, error_cap);
    }
