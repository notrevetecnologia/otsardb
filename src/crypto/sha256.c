...
#if defined(__EMSCRIPTEN__)
    /* WASM build: pure-C SHA-256 (no OpenSSL EVP). Byte-identical to the
     * OpenSSL path (validated against NIST vectors + a native cross-check —
     * project audit trail). */
    return otsardb_sha256_pure_digest(data, size, out);
#else
