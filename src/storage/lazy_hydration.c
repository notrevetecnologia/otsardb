...
#if defined(_WIN32)
static int lazy_list_cmp_msvc(void *arg, const void *a, const void *b) {
    lazy_list_cmp_ctx *ctx = (lazy_list_cmp_ctx *)arg;
    return lazy_list_cmp_impl(ctx->slots, (const size_t *)a, (const size_t *)b);
}
#else
static int lazy_list_cmp_glibc(const void *a, const void *b, void *arg) {
    lazy_list_cmp_ctx *ctx = (lazy_list_cmp_ctx *)arg;
    return lazy_list_cmp_impl(ctx->slots, (const size_t *)a, (const size_t *)b);
}
#endif
...
static int lazy_try_list_chain(otsardb_object_store *store,
    ...
    if (all_fetched) {
        ...
        if (!order) all_fetched = 0;
            ...
            lazy_list_cmp_ctx cmp_ctx = {slots};
#if defined(_WIN32)
            qsort_s(order, collect.count, sizeof(*order), lazy_list_cmp_msvc, &cmp_ctx);
#else
            qsort_r(order, collect.count, sizeof(*order), lazy_list_cmp_glibc, &cmp_ctx);
#endif
        }
    }
