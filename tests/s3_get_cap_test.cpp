...
void handle_request(socket_t sock) {
    ...
    while (sent < body_size) {
        ...
        /* Throttle the huge-body stream so the client's cap-abort reliably
         * happens before the loopback socket buffer swallows the whole body.
         * Without this, the server's blocking send() can drain the full body
         * into the kernel buffer before the client's datafunc aborts, and the
         * "client did not abort early" assertion fails on fast loopback (seen
         * on the GitHub Actions Windows runner). The engine behavior is
         * unchanged: the client still refuses and never buffers the body. */
        if (g_mode.load()) std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    delete[] fill;
...
int main(void) {
#if !defined(_WIN32)
    /* The server thread sends to the client after it aborts; on macOS the
     * default SIGPIPE action kills the whole process instead of making
     * send() return EPIPE. Ignore it so the cap-abort is observable. */
